...
from xclib.utils.sparse import compute_centroid, retain_topk
...
class build_tree:
    ...
    def fit(self, lbl_repr, labels = None, dataset = None):
        # lbl_repr = _normalize(lbl_repr)

        if labels is not None: #ASTEC
            freq = lbl_repr.getnnz(axis = 0)
            if dataset == 'AmazonTitles-3M':
            # if freq.max() > 5000:
                print("Correlation matrix is too dense. Skipping..")
            else:
                print("Using Random Walk to computer correlations")
                cooc = scale(compute_correlation(labels), norm="l1")
                lbl_repr = cooc.dot(lbl_repr)
